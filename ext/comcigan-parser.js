...
class Timetable {
  ...
  async init(option) {
    if (option) {
      ...
      if (option["tempSave"] !== undefined) {
        console.log(
          "[comcigan-parser] WARNING: tempSave option was deprecated at >= 0.1.0"
        );
      }
    } else {
      this._option = {
    ...
    await new Promise((resolve, reject) => {
      internetConnectionCheck().catch(why => reject(why))
  ...
  async setSchool(keyword) {
    ...
    let hexString = "";
    hexString = encodeURIforEUCKR(keyword)

    await new Promise((resolve, reject) => {
      fetch(
        this._baseUrl + this._extractCode + hexString
      ).then((res) => {
        console.log(this._baseUrl + this._extractCode + hexString)
        if (!res.ok) {
          reject(res.statusText)
        }
        res.text().then((body) => {
  ...
  async getTimetable() {
    ...
    const sc3 =
      ...
      "?" +
      btoa(s7 + "_" + da1 + "_" + this._scData[2]);

    // JSON 데이터 로드
    const resultJson = await new Promise((resolve, reject) => {
      fetch(this._baseUrl + sc3).then((res) => {
        if (!res.ok) {
          reject(res.statusText)
        }
        res.text().then((body) => {
    ...
    for (let grade = 1; grade <= this._option["maxGrade"]; grade++) {
      ...
      for (let classNum = 1; classNum <= classCount[grade]; classNum++) {
        ...
        for (let weekday = 1; weekday <= 5; weekday++) {
          // 1교시 ~ 해당 날짜의 수업 교시
          for (
            let classTime = 1; classTime <= time[grade][weekday]; classTime++
          ) {
            ...
            if (!(classNum in timetableData[grade])) {
              timetableData[grade][classNum] = Array.apply(
                null,
                new Array(5)
              ).map(() => []);
            }
